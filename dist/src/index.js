...
function typeToTs(type) {
    ...
    if (n === "int64" || n === "uint64")
        return "bigint";
    if (["int8", "int16", "int32", "uint8", "uint16", "uint32", "integer", "float32", "float64", "float", "decimal"].includes(n))
        return "number";
    if (n === "bytes")
        return "Uint8Array";
    if (type.kind === "Intrinsic" && type.name === "string")
        return "string";
    if (type.kind === "Intrinsic" && type.name === "boolean")
        return "boolean";
    if (type.kind === "Model" && type.indexer)
        return `${typeToTs(type.indexer.value)}[]`;
    if (type.kind === "Model")
...
function writeJsonExpr(type, varExpr) {
    ...
    if (n === "int32" || n === "int8" || n === "int16" || n === "integer")
        return `w.writeInt32(${varExpr})`;
    if (n === "int64")
        return `w.writeInt64(${varExpr})`;
    if (n === "uint32" || n === "uint8" || n === "uint16")
        return `w.writeUint32(${varExpr})`;
    if (n === "uint64")
    ...
    if (type.kind === "Model" && type.name)
        return `${type.name}Codec.writeJson(w, ${varExpr})`;
...
function writeMsgPackExpr(type, varExpr) {
    ...
    if (type.kind === "Model" && type.name)
        return `${type.name}Codec.writeMsgPack(w, ${varExpr})`;
    return `w.writeString(String(${varExpr}))`;
}
function readJsonExpr(type) {
    const n = scalarName(type);
    if (n === "string")
        return `r.readString()`;
    if (n === "boolean")
